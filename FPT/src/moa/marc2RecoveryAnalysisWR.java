...
public class marc2RecoveryAnalysisWR {
...
*/			}
			...
			folder="D:\\RecoveryAnaysis\\classification\\copy\\" ;
			//folder="" ;
			...
			if (regression)
			{
				...
				if (!drift)
				{
					stream = " -s (moa.tasks.CacheShuffledWReplacementStream -s (ArffFileStream -f "+folder+startStream+" ) -r "+seed+") " ;
					resFile = folder+algorithm+startStream+seed +".csv" ;
					finalPart= " -i "+length+" -f "+width+" -d "+resFile ;
				}
				else
				{				
					stream = " -s (ConceptDriftStream -s (CacheShuffledWReplacementStream -s (ArffFileStream -f "+folder+startStream+" ) -r "+seed+")  " +
					" -d (CacheShuffledWReplacementStream -s (ArffFileStream -f "+folder+endStream+" ) -r "+seed+") -p "+driftMid+" -w "+driftWidth+" ) " ;
